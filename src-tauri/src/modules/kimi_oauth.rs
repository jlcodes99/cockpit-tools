...
pub const X_MSH_PLATFORM: &str = "kimi_cli";
pub const X_MSH_VERSION: &str = "0.38.0";
...
pub fn expand_kimi_home_path(raw: &str) -> std::path::PathBuf {
...
pub fn default_kimi_home() -> Result<std::path::PathBuf, String> {
    ...
    if let Some(configured) = crate::modules::config::get_user_config()
        .kimi_code_home
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
    {
        return Ok(expand_kimi_home_path(configured));
    }
...
pub fn ensure_device_id(home: &std::path::Path) -> Result<String, String> {
    ensure_device_id_with(home, None)
}

pub fn ensure_device_id_with(
    home: &std::path::Path,
...
pub async fn start_login() -> Result<KimiOAuthStartResponse, String> {
    ...
    let interval_seconds = device.interval.unwrap_or(DEFAULT_INTERVAL_SECONDS).max(1);
...
mod tests {
    ...
    fn needs_refresh_false_without_expires_at() {
        assert!(!needs_refresh(None, Some(3600)));
    }

    #[test]
    fn default_kimi_home_respects_env_override() {
        let _guard = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let _home = KimiHomeGuard::new("override");
        let home = default_kimi_home().expect("home");
        assert_eq!(
            home,
    ...
    #[test]
    fn default_kimi_home_respects_config_when_env_unset() {
        let _guard = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let _lock = crate::modules::test_support::env_lock()
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let dir = DataDirGuard::new("home-cfg");
        let previous = std::env::var("KIMI_CODE_HOME").ok();
        std::env::remove_var("KIMI_CODE_HOME");
        let configured = dir.dir.join("custom-kimi-home");
        crate::modules::config::set_kimi_code_home(Some(configured.to_string_lossy().into_owned()))
            .expect("set home");
        let home = default_kimi_home().expect("home");
        match previous {
            Some(value) => std::env::set_var("KIMI_CODE_HOME", value),
        ...
        assert_eq!(home, configured);
    }

    #[test]
    fn ensure_device_id_writes_preferred_when_missing() {
        let _guard = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = KimiHomeGuard::new("preferred-id");
        let id = ensure_device_id_with(&home.dir, Some("account-device-id")).expect("write");
        assert_eq!(id, "account-device-id");
        let again = ensure_device_id_with(&home.dir, Some("other-device-id")).expect("keep");
        assert_eq!(again, "account-device-id");
    }

    #[test]
    fn ensure_device_id_is_stable() {
        let _guard = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let _home = KimiHomeGuard::new("device-id");
        let home = default_kimi_home().expect("home");
        let first = ensure_device_id(&home).expect("first");
        let second = ensure_device_id(&home).expect("second");
        assert_eq!(first, second);
        assert!(!first.is_empty());
    }

    #[test]
    fn cancel_login_without_pending_is_ok() {
        let _guard = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let _lock = crate::modules::test_support::env_lock()
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let _dir = DataDirGuard::new("cancel-none");
        cancel_login(None).expect("cancel noop");
    }

    #[tokio::test]
    async fn refresh_token_rejects_empty_refresh() {
        let err = refresh_token("", None).await.expect_err("empty refresh");
