...
/// Create system tray (full menu, including account data loading)
/// Create skeleton tray (no account file I/O, basic menu items only, for fast startup)
...
fn build_devin_cli_display_info(lang: &str) -> AccountDisplayInfo {
    ...
    let mut quota_lines = vec![
        ...
        format!(
            "Status: {}",
            if needs_login { "Needs login" } else { "Ready" }
        ),
    ];
