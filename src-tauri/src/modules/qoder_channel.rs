    ...
    pub fn default_exe_candidates(self) -> Vec<PathBuf> {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            match self {
                ...
                Self::QoderCnApp => {
                    if let Some(ref pd) = program_data {
                        let ini_file1 = pd.join("Qoder CN").join("Qoder CN Launcher").join("install.ini");
                        if let Some((dir, exe)) = parse_launcher_install_ini(&ini_file1) {
                            candidates.push(PathBuf::from(dir).join(exe));
                        }
                        let ini_file2 = pd.join("Qoder CN").join("install.ini");
                        if let Some((dir, exe)) = parse_launcher_install_ini(&ini_file2) {
                            candidates.push(PathBuf::from(dir).join(exe));
                        }
                    }
                    if let Some(ref pf) = program_files {
                    ...
                    if let Some(ref pd) = program_data {
                        candidates.push(pd.join("Qoder CN").join("Qoder CN Launcher").join("Qoder Launcher.exe"));
