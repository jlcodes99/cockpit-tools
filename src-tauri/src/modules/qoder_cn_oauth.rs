...
use crate::modules::{logger, qoder_cn_account};
...
fn build_cli_device_login_url(
    ...
    let mut url = Url::parse(login_base_url)
        .map_err(|err| format!("解析 Qoder CN 登录地址失败: {}", err))?;
    {
...
fn build_reqwest_client() -> Result<reqwest::Client, String> {
    reqwest::Client::builder()
        ...
        .build()
        .map_err(|err| format!("创建 Qoder CN HTTP 客户端失败: {}", err))
}
...
async fn poll_device_token_once(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|err| format!("轮询 Qoder CN device token 失败: {}", err))?;
    ...
    if !response.status().is_success() {
        ...
        return Err(format!(
            "轮询 Qoder CN device token 失败: status={}, body_len={}",
            status,
            body.len()
        ));
    }
    ...
    let payload = response
        ...
        .await
        .map_err(|err| format!("解析 Qoder CN device token 响应失败: {}", err))?;
    if payload
        .token
        .as_deref()
...
async fn fetch_user_info(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|err| format!("请求 Qoder CN userinfo 失败: {}", err))?;

    if !response.status().is_success() {
        let status = response.status();
        let body = response.text().await.unwrap_or_default();
        return Err(format!("请求 Qoder CN userinfo 失败: status={}, body_len={}", status, body.len()));
    }
...
async fn fetch_credit_usage(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|err| format!("请求 Qoder CN quota/usage 失败: {}", err))?;

    if !response.status().is_success() {
        let status = response.status();
        let body = response.text().await.unwrap_or_default();
        return Err(format!("请求 Qoder CN quota/usage 失败: status={}, body_len={}", status, body.len()));
    }
...
pub async fn start_login() -> Result<QoderCnOAuthStartResponse, String> {
    logger::log_info("[QoderCN OAuth] 开始创建登录会话");
    ...
    logger::log_info(&format!(
        "[QoderCN OAuth] 已生成 device login 链接: login_id={}, verification_uri_len={}",
        login_id,
...

    {
        let mut guard = PENDING_OAUTH_STATE
            .lock()
            .map_err(|_| "获取 Qoder CN OAuth 状态锁失败".to_string())?;
        *guard = Some(state);
    }

    Ok(QoderCnOAuthStartResponse {
        login_id,
        verification_uri,
...
pub async fn complete_login(login_id: &str) -> Result<QoderCnAccount, String> {
    logger::log_info(&format!(
        "[QoderCN OAuth] 开始等待登录完成: login_id={}",
        login_id
    ));
    ...
    loop {
        let snapshot = {
            ...
            let state = guard
                .as_ref()
                .ok_or_else(|| "没有进行中的 Qoder CN OAuth 登录会话".to_string())?;

            if state.login_id != login_id {
                return Err("Qoder CN OAuth 登录会话已变更，请重新发起".to_string());
            }
            if state.cancelled {
                return Err("Qoder CN OAuth 登录已取消".to_string());
            }
            if now_timestamp() > state.expires_at {
                clear_pending_if_matches(login_id);
                return Err(
                    last_poll_error.unwrap_or_else(|| "Qoder CN OAuth 登录已超时，请重试".to_string())
                );
            }
            ...
            .await
        {
            Ok(Some(token_data)) => {
                logger::log_info(&format!(
                    "[QoderCN OAuth] deviceToken/poll 命中: login_id={}, elapsed={}ms",
                    login_id,
                    wait_started.elapsed().as_millis()
                ));

                let access_token = normalize_non_empty(token_data.token.as_deref())
                    .ok_or_else(|| "Qoder CN device token 响应缺少 token".to_string())?;
                ...
                let quota_raw = match fetch_credit_usage(&client, &snapshot.3, &access_token).await {
                    ...
                    Err(err) => {
                        logger::log_warn(&format!(
                            "[QoderCN OAuth] 获取 quota/usage 失败: {}",
                            err
                        ));
                        None
                    }
                ...
                let account = qoder_cn_account::upsert_account_from_payload(
                    email,
                    final_user_id,
                    display_name,
                ...
                // 主动写入 Qoder CN 认证文件（auth-v2.dat, .auth-cn/id, .auth-cn/user, .status.json）
                // 这样备份时就能捕获完整的会话数据，切换后 Qoder CN 能保持登录状态
                let avatar_url = user_info.as_ref().and_then(|info| {
                    info.get("avatar").and_then(|v| v.as_str()).map(|s| s.to_string())
                });
                if let Err(err) = qoder_cn_account::write_oauth_session_files(
                    &access_token,
                    ...
                    logger::log_warn(&format!(
                        "[QoderCN OAuth] 写入会话文件失败: {}",
                        err
                    ));
                }

                // 备份当前会话文件到新创建的账号
                if let Err(err) = qoder_cn_account::backup_current_session_to(&account.id) {
                    logger::log_warn(&format!(
                        "[QoderCN OAuth] 备份会话失败: account_id={}, error={}",
                        account.id, err
                    ));
                } else {
                    logger::log_info(&format!(
                        "[QoderCN OAuth] 会话备份完成: account_id={}",
                        account.id
                    ));
                }
                ...
                clear_pending_if_matches(login_id);
                logger::log_info(&format!(
                    "[QoderCN OAuth] 登录完成: login_id={}, account_id={}, email={}",
            ...
            Err(err) => {
                ...
                logger::log_warn(&format!(
                    "[QoderCN OAuth] deviceToken/poll 失败: login_id={}, error={}",
                    login_id, err
                ));
            }
        }
        ...
        if elapsed >= next_wait_log_at {
            logger::log_info(&format!(
                "[QoderCN OAuth] 等待 device token 中: login_id={}, elapsed={}s",
                login_id,
                elapsed.as_secs()
            ));
...
pub fn peek_pending_login() -> Option<QoderCnOAuthStartResponse> {
    ...
    Some(QoderCnOAuthStartResponse {
...
pub async fn build_account_from_token(token: &str) -> Result<QoderCnAccount, String> {
    let client = build_reqwest_client()?;
    ...
    let user_info = match fetch_user_info(&client, DEFAULT_OPENAPI_BASE_URL, token).await {
        ...
        Err(err) => {
            logger::log_warn(&format!(
                "[QoderCN OAuth] token 导入获取 userinfo 失败: {}",
                err
            ));
            None
        }
    ...
    let quota_raw = match fetch_credit_usage(&client, DEFAULT_OPENAPI_BASE_URL, token).await {
        ...
        Err(err) => {
            logger::log_warn(&format!(
                "[QoderCN OAuth] token 导入获取 quota/usage 失败: {}",
                err
            ));
            None
        }
    ...
    qoder_cn_account::upsert_account_from_payload(
        email,
        user_id,
        display_name,
...
/// 刷新单个账号的 token 和配额
pub async fn refresh_account_from_openapi(account_id: &str) -> Result<QoderCnAccount, String> {
    let result = refresh_account_from_openapi_once(account_id).await;
    if let Err(err) = &result {
        let _ = qoder_cn_account::update_quota_query_error(account_id, Some(err.clone()));
    }
    result
}

async fn refresh_account_from_openapi_once(account_id: &str) -> Result<QoderCnAccount, String> {
    let target = qoder_cn_account::load_account(account_id)
    ...
    let access_token = target
        ...
        .as_ref()
        .filter(|t| !t.is_empty())
        .ok_or_else(|| "Qoder CN 账号缺少 access token，请重新登录后再刷新".to_string())?
        .clone();
    ...
    if let Some(refresh_token) = target.refresh_token.as_ref().filter(|t| !t.is_empty()) {
        match refresh_token_api(&client, DEFAULT_OPENAPI_BASE_URL, refresh_token).await {
            ...
            Err(err) => {
                logger::log_warn(&format!(
                    "[QoderCN Refresh] token 刷新失败，将使用现有 token: {}",
                    err
                ));
            }
        }
    ...
    let user_info = match fetch_user_info(&client, DEFAULT_OPENAPI_BASE_URL, &current_token).await {
        Ok(value) => Some(value),
        Err(err) => {
            logger::log_warn(&format!("[QoderCN Refresh] 获取 userinfo 失败: {}", err));
            None
        }
    };
    ...
    let quota_raw = match fetch_credit_usage(&client, DEFAULT_OPENAPI_BASE_URL, &current_token).await {
        Ok(value) => Some(value),
        Err(err) => {
            logger::log_warn(&format!("[QoderCN Refresh] 获取 quota/usage 失败: {}", err));
    ...
    let refreshed = qoder_cn_account::upsert_account_from_payload(
        email,
        user_id,
        display_name,
        Some(current_token),
    ...
    if refreshed.id == target.id {
        if let Some(error_msg) = quota_query_error {
            if let Some(updated) =
                qoder_cn_account::update_quota_query_error(&refreshed.id, Some(error_msg))?
            {
                return Ok(updated);
            }
        }
...
pub async fn refresh_all_accounts_from_openapi() -> Result<i32, String> {
    let accounts = qoder_cn_account::list_accounts();
    ...
    for account in &accounts {
        ...
        match refresh_account_from_openapi(&account.id).await {
            ...
            Err(err) => {
                logger::log_warn(&format!(
                    "[QoderCN Refresh] 刷新账号失败: id={}, error={}",
                    account.id, err
                ));
            }
        }
