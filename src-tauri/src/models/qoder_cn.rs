...
pub struct QoderCnAccount {
    ...
    /// Raw encrypted JSON from state.vscdb for auth.userInfo
    #[serde(skip_serializing_if = "Option::is_none", rename = "auth_user_info_raw")]
    pub auth_user_info_raw: Option<Value>,
    /// Raw encrypted JSON from state.vscdb for auth.userPlan
    #[serde(skip_serializing_if = "Option::is_none", rename = "auth_user_plan_raw")]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QoderCnAccountSummary {
    pub id: String,
    pub email: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub user_id: Option<String>,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QoderCnAccountIndex {
    pub version: String,
    pub accounts: Vec<QoderCnAccountSummary>,
}

impl QoderCnAccountIndex {
    pub fn new() -> Self {
        Self {
...
impl Default for QoderCnAccountIndex {
    fn default() -> Self {
        Self::new()
    }
}
...
pub struct QoderCnOAuthStartResponse {
    pub login_id: String,
...
pub struct AuthV2Data {
    ...
    /// Qoder CN uses camelCase "refreshToken"; alias for legacy snake_case reads
    #[serde(
    ...
    pub refresh_token: Option<String>,
    /// Qoder CN uses camelCase "expiresAt"; alias for legacy snake_case reads
    #[serde(
        skip_serializing_if = "Option::is_none",
        rename = "expiresAt",
        alias = "expires_at"
    ...
    /// Required by Qoder CN: must be 2
...
impl QoderCnAccount {
    pub fn summary(&self) -> QoderCnAccountSummary {
        QoderCnAccountSummary {
            id: self.id.clone(),
