...
export function CodexAccountsPage() {
  ...
  const renderGridCards = (items: typeof filteredAccounts, groupKey?: string) =>
    items.map((account) => {
      ...
      return (
        <div key={groupKey ? `${groupKey}-${account.id}` : account.id} className={`account-card codex-account-card ${isCurrent ? 'current' : ''} ${isSelected ? 'selected' : ''} ${isNewApiAccount ? "new-api-exclusive" : ""}`}>
            ...
            {isApiKeyAccount ? (
              ...
              <>
                ...
                {quotaItems.map((item) => {
                  ...
                  return (<div key={item.key} className="quota-compact-item"><div className="quota-compact-header"><QuotaIcon size={14} /><span className="model-label">{item.label}</span><span className={`model-pct ${item.quotaClass}`}>{item.valueText}</span></div>
