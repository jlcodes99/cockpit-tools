...
const TRAE_CN_FLOW_NOTICE_COLLAPSED_KEY = 'agtools.trae_cn.flow_notice_collapsed';
const TRAE_CN_FILTER_PERSISTENCE_SCOPE = normalizeAccountsOverviewScope('Trae CN');
...
export function TraeCnAccountsPage() {
  ...
  const [filterTypes, setFilterTypes] = useState<string[]>(() =>
    readAccountsOverviewFilterPersistenceEnabled(TRAE_CN_FILTER_PERSISTENCE_SCOPE)
      ? readAccountsOverviewFilterStringArray(TRAE_CN_FILTER_PERSISTENCE_SCOPE, FILTER_TYPES_FIELD)
      : [],
  );
  const untaggedKey = '__untagged__';

  const store = useTraeCnAccountStore();

  useEffect(() => {
    let cancelled = false;
    traeCnService
      .getTraeCnLaunchOnSwitch()
      .then((config) => {
        if (!cancelled) {
          setLaunchOnSwitch(config.enabled);
        }
      })
      .catch((error) => {
        console.error('[Trae CN] Failed to load launch-on-switch config:', error);
      });
    return () => {
      cancelled = true;
    };
  ...
  const page = useProviderAccountsPage<TraeAccount>({
    platformKey: 'Trae CN',
    oauthLogPrefix: 'TraeCnOAuth',
    flowNoticeCollapsedKey: TRAE_CN_FLOW_NOTICE_COLLAPSED_KEY,
    ...
    oauthService: {
      startLogin: traeCnService.traeCnOauthLoginStart,
      completeLogin: (loginId: string) => traeCnService.traeCnOauthLoginComplete(loginId),
      cancelLogin: (loginId?: string) => traeCnService.traeCnOauthLoginCancel(loginId),
      submitCallbackUrl: (loginId: string, callbackUrl: string) =>
        traeCnService.traeCnOauthSubmitCallbackUrl(loginId, callbackUrl),
    },
    dataService: {
    ...
    resolveInjectSuccessDisplayText: (account) => getTraeAccountDisplayName(account),
  });
  ...
  const handleLaunchOnSwitchChange = useCallback(async (enabled: boolean) => {
    const previous = launchOnSwitch;
    setLaunchOnSwitch(enabled);
    setLaunchOnSwitchSaving(true);
    try {
      const config = await traeCnService.setTraeCnLaunchOnSwitch(enabled);
      setLaunchOnSwitch(config.enabled);
      window.dispatchEvent(new Event('config-updated'));
    } catch (error) {
      console.error('[Trae CN] Failed to save launch-on-switch config:', error);
      setLaunchOnSwitch(previous);
      setMessage({
        tone: 'error',
        text: `保存 Trae CN 切换启动配置失败: ${String(error)}`,
      });
    } finally {
      setLaunchOnSwitchSaving(false);
    }
  }, [launchOnSwitch, setMessage]);
        ...
        <>
          <div className="trae-cn-switch-config" role="group" aria-label="Trae CN 切号启动配置">
            <div className="trae-cn-switch-config-copy">
              <div className="trae-cn-switch-config-title">
                {t('traeCn.settings.launchOnSwitch', '切换后启动/重启 Trae CN')}
              </div>
              <div className="trae-cn-switch-config-desc">
                {t(
                  'traeCn.settings.launchOnSwitchDesc',
                  '开启后，切换账号会尝试关闭并重新打开 Trae CN，让客户端读取新的登录态。',
                )}
              </div>
            </div>
            <label className="trae-cn-switch-toggle">
              <input
                type="checkbox"
                checked={launchOnSwitch}
                disabled={launchOnSwitchSaving}
                onChange={(event) => handleLaunchOnSwitchChange(event.target.checked)}
                aria-label={t('traeCn.settings.launchOnSwitch', '切换后启动/重启 Trae CN')}
              />
              <span className="trae-cn-switch-slider" />
            </label>
          </div>
          ...
          {loading && accounts.length === 0 ? (
            ...
            <div className="empty-state">
              ...
              <p>
                {t(
                  'traeCn.empty.description',
                  '点击“添加账号”开始管理您的 Trae CN 账号，也可以从本机或 JSON 文件导入。',
                )}
              </p>
            ...
            <div className="modal-overlay">
              <div className="modal-content ghcp-add-modal" onClick={(event) => event.stopPropagation()}>
                <div className="modal-header">
                  ...
                  <h2>{t('traeCn.addModal.title', '添加 Trae CN 账号')}</h2>
                  <button
                  ...
                  {addTab === 'oauth' ? (
                    <div className="add-section">
                      <p className="section-desc">
                        {t('traeCn.oauth.desc', '点击下方按钮，在浏览器中完成 Trae CN OAuth 授权登录。')}
                      </p>
                    ...
                    <div className="add-section">
                      <p className="section-desc">
                        {t(
                          ...
                          '粘贴完整 Trae CN JSON。暂不支持裸 token，避免导入后无法稳定识别账号或写入不可用登录态。',
                        )}
                      </p>
                      <textarea
                        ...
                        placeholder={t('traeCn.token.placeholder', '粘贴完整 Trae CN JSON...')}
                      />
                      <button
                    ...
                    <div className="add-section">
                      <p className="section-desc">
                        {t('traeCn.import.localDesc', '从本机 Trae CN 客户端读取已登录账号，或导入由本工具导出的 Trae CN JSON 文件。')}
                      </p>
                      <button
