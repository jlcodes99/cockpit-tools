...
import {
  ...
  getQoderworkCnSubscriptionInfo,
  getQoderworkCnUsage,
  hasQoderworkCnQuotaData,
...
const QODERWORK_CN_FLOW_NOTICE_COLLAPSED_KEY = 'agtools.qoderwork_cn.flow_notice_collapsed';
const QODERWORK_CN_CURRENT_ACCOUNT_ID_KEY = 'agtools.qoderwork_cn.current_account_id';
const QODERWORK_CN_FILTER_PERSISTENCE_SCOPE = normalizeAccountsOverviewScope('qoderwork_cn');
...
function resolveQoderworkCnPlanBadgeClass(plan: string): string {
...
type QoderworkCnQuotaDisplayItem = {
  key: string;
  label: string;
...
type QoderworkCnQuotaDisplay = {
  planTag: string;
  planClass: string;
  items: QoderworkCnQuotaDisplayItem[];
  resetText: string | null;
};

export function QoderworkCnAccountsPage() {
  const { t } = useTranslation();
  const [activeTab, setActiveTab] = useState<PlatformOverviewTab>('overview');
  const store = useQoderworkCnAccountStore();

  const [filterTypes, setFilterTypes] = useState<string[]>(() =>
    readAccountsOverviewFilterPersistenceEnabled(QODERWORK_CN_FILTER_PERSISTENCE_SCOPE)
      ? readAccountsOverviewFilterStringArray(QODERWORK_CN_FILTER_PERSISTENCE_SCOPE, FILTER_TYPES_FIELD)
      : [],
  );

  const page = useProviderAccountsPage<QoderworkCnAccount>({
    platformKey: 'QoderWork CN',
    oauthLogPrefix: 'QoderworkCnOAuth',
    flowNoticeCollapsedKey: QODERWORK_CN_FLOW_NOTICE_COLLAPSED_KEY,
    currentAccountIdKey: QODERWORK_CN_CURRENT_ACCOUNT_ID_KEY,
    exportFilePrefix: 'qoderwork_cn_accounts',
    oauthTabKeys: ['oauth'],
    store: {
    ...
    oauthService: {
      startLogin: qoderworkCnService.qoderworkCnOauthLoginStart,
      completeLogin: qoderworkCnService.qoderworkCnOauthLoginComplete,
      cancelLogin: qoderworkCnService.qoderworkCnOauthLoginCancel,
    },
    dataService: {
      importFromJson: qoderworkCnService.importQoderworkCnFromJson,
      importFromLocal: qoderworkCnService.importQoderworkCnFromLocal,
      addWithToken: qoderworkCnService.addQoderworkCnAccountWithToken,
      exportAccounts: qoderworkCnService.exportQoderworkCnAccounts,
      injectToVSCode: qoderworkCnService.switchQoderworkCnAccount,
    },
    getDisplayEmail: (account) => getQoderworkCnAccountDisplayEmail(account),
  });
  ...
  const isAbnormalAccount = useCallback((_account: QoderworkCnAccount) => false, []);

  const tierSummary = useMemo(() => {
    const counts = new Map<string, number>();
    counts.set('UNKNOWN', 0);
    for (const account of accounts) {
      const plan = getQoderworkCnPlanBadge(account) || 'UNKNOWN';
  ...
  const filteredAccounts = useMemo(() => {
    ...
    if (query) {
      result = result.filter((account) => {
        const text = `${getQoderworkCnAccountDisplayEmail(account)} ${account.user_id || ''} ${getQoderworkCnPlanBadge(account)}`.toLowerCase();
    ...
    if (filterTypes.length > 0) {
      ...
      if (selectedTypes.size > 0) result = result.filter((a) => selectedTypes.has(getQoderworkCnPlanBadge(a)));
    }
    ...
    result.sort((a, b) => {
      ...
      if (sortBy === 'plan') {
        const cmp = getQoderworkCnPlanBadge(a).localeCompare(getQoderworkCnPlanBadge(b));
        return sortDirection === 'asc' ? cmp : -cmp;
      }
      if (sortBy === 'quota') {
        const left = getQoderworkCnUsage(a).usagePercent ?? -1;
        const right = getQoderworkCnUsage(b).usagePercent ?? -1;
  ...
  const pagination = usePagination({
    ...
    storageKey: buildPaginationPageSizeStorageKey('QoderWork CN'),
  });
  ...
  const resolveQuotaDisplay = useCallback(
    (account: QoderworkCnAccount): QoderworkCnQuotaDisplay => {
      ...
      return {
        ...
        planClass: resolveQoderworkCnPlanBadgeClass(subscription.planTag),
        items: [
          buildItem('user_quota', t('qoderworkCn.quota.userQuota', '用户配额'), subscription.userQuota.used, subscription.userQuota.total, subscription.userQuota.percentage),
          buildItem('add_on', t('qoderworkCn.quota.addOn', '加量包'), subscription.addOnQuota.used, subscription.addOnQuota.total, subscription.addOnQuota.percentage),
          buildItem('org_pkg', t('qoderworkCn.quota.orgPackage', '组织资源包'), subscription.orgResourcePackage.used, subscription.orgResourcePackage.total, subscription.orgResourcePackage.percentage),
        ],
        resetText: subscription.expiresAt != null
          ? t('qoderworkCn.quota.resetAt', '订阅重置：{{date}}', { date: formatDisplayDate(subscription.expiresAt) })
          : null,
      };
    },
    [t],
  ...
  const renderQuotaSection = useCallback(
    (account: QoderworkCnAccount) => {
      if (!hasQoderworkCnQuotaData(account)) {
        return (
  ...
  const renderGridCards = (items: typeof filteredAccounts, groupKey?: string) =>
    items.map((account) => {
      const maskedEmail = maskAccountText(getQoderworkCnAccountDisplayEmail(account));
      const isCurrent = currentAccountId === account.id;
      const isSelected = selected.has(account.id);
      const accountTags = (account.tags || []).map((tag) => tag.trim()).filter(Boolean);
      const visibleTags = accountTags.slice(0, 2);
      const moreTagCount = Math.max(0, accountTags.length - visibleTags.length);
      const plan = getQoderworkCnPlanBadge(account);
      const planClass = resolveQoderworkCnPlanBadgeClass(plan);
        ...
        <div key={groupKey ? `${groupKey}-${account.id}` : account.id}
          ...
          <div className="card-footer">
            ...
            <div className="card-actions">
              ...
              <button className="card-action-btn export-btn" onClick={() => void handleExportByIds([account.id], getQoderworkCnAccountDisplayEmail(account))}
    ...
    items.map((account) => {
      ...
      return (
        <tr key={groupKey ? `${groupKey}-${account.id}` : account.id} className={isCurrent ? 'current' : undefined}>
          ...
          <td title={maskAccountText(getQoderworkCnAccountDisplayEmail(account))}>
            <div className="account-cell">
              <div className="account-main-line">{maskAccountText(getQoderworkCnAccountDisplayEmail(account))}</div>
              {quotaError && (
  ...
  return (
    <div className="ghcp-accounts-page qoder-accounts-page qoderwork-cn-accounts-page">
      <PlatformOverviewTabsHeader platform="qoderwork_cn" active={activeTab} onTabChange={setActiveTab} />
        ...
        <>
          <div className={`ghcp-flow-notice ${isFlowNoticeCollapsed ? 'collapsed' : ''}`} role="note" aria-live="polite">
            <button type="button" className="ghcp-flow-notice-toggle"
              ...
              <div className="ghcp-flow-notice-title">
                <CircleAlert size={16} />
                <span>{t('qoderworkCn.flowNotice.title', 'QoderWork CN 账号管理说明（点击展开/收起）')}</span>
              </div>
            ...
            {!isFlowNoticeCollapsed && (
              <div className="ghcp-flow-notice-body">
                <div className="ghcp-flow-notice-desc">
                  {t('qoderworkCn.flowNotice.desc', '切换账号将备份当前会话、关闭 QoderWork CN 应用、恢复目标账号会话并重启应用。数据仅在本地处理。')}
                </div>
                <ul className="ghcp-flow-notice-list">
                  <li>{t('qoderworkCn.flowNotice.permission', '权限范围：读取 QoderWork CN 本地认证存储（auth.dat, auth-v2.dat），用于账号切换与会话备份恢复。')}</li>
                  <li>{t('qoderworkCn.flowNotice.network', '网络范围：OAuth 授权登录与 Token 刷新需联网请求 qoder.com.cn 与 openapi.qoder.com.cn；配额查询通过 OpenAPI 获取用量数据。不上传本地密钥或凭证。')}</li>
                </ul>
              </div>
            )}
          </div>
          ...
          <div className="toolbar">
            <div className="toolbar-left">
              <div className="search-box">
                ...
                <input type="text" value={searchQuery}
                  placeholder={t('qoderworkCn.search', '搜索 QoderWork CN 账号...')}
            ...
            <div className="toolbar-right">
              ...
              <QuickSettingsPopover type="qoderwork_cn" />
            </div>
          </div>
          ...
          {loading && accounts.length === 0 ? (
            ...
            <div className="empty-state">
              ...
              <p>{t('qoderworkCn.empty.desc', '点击"添加账号"，可使用授权登录、本机导入或 JSON 导入。')}</p>
        ...
        <div className="modal-overlay">
          <div className="modal-content codex-add-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <h2>{t('qoderworkCn.addModal.title', '添加 QoderWork CN 账号')}</h2>
            ...
            <div className="modal-body">
              ...
              {addTab === 'oauth' ? (
                <div className="add-section">
                  <p className="section-desc">{t('qoderworkCn.oauthDesc', '点击下方按钮，在浏览器中完成 QoderWork CN 账号 OAuth 授权。')}</p>
                  {oauthPrepareError ? (
                    ...
                    <div className="oauth-url-section">
                      ...
                      {oauthPolling && (
                        <div className="add-status loading">
                          ...
                          <span>{t('qoderworkCn.oauthWaiting', '等待授权完成...')}</span>
                        </div>
                      )}
                      {oauthCompleteError && (
                        <div className="add-status error">
                ...
                <div className="add-section">
                  <p className="section-desc">{t('qoderworkCn.tokenDesc', '粘贴 QoderWork CN 的 access token：')}</p>
                ...
                <div className="add-section">
                  <p className="section-desc">{t('qoderworkCn.import.localDesc', '支持从本机 QoderWork CN 客户端或 JSON 文件导入账号数据。')}</p>
                  <button className="btn btn-secondary btn-full" onClick={() => handleImportFromLocal?.()} disabled={importing}>
                    ...
                    {t('qoderworkCn.import.localClient', '从本机 QoderWork CN 导入')}
                  </button>
