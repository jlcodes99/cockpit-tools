export interface QoderCnAccount {
  id: string;
  email: string;
  user_id?: string | null;
...
export interface QoderCnSubscriptionInfo {
  planTag: string;
  userType: string | null;
  userQuota: QoderCnQuotaBucket;
  addOnQuota: QoderCnQuotaBucket;
  orgResourcePackage: QoderCnQuotaBucket;
  totalUsagePercentage: number | null;
  expiresAt: number | null;
  isQuotaExceeded: boolean;
}

export interface QoderCnUsageOverview {
  planTag: string;
  usagePercent: number | null;
  creditsUsed: number | null;
  creditsTotal: number | null;
...
const QODER_CN_SENTINEL_EXPIRES_AT_MS = Date.UTC(9999, 11, 31, 0, 0, 0);
...
function normalizeTimestampMs(value: unknown): number | null {
  ...
  if (timestampMs >= QODER_CN_SENTINEL_EXPIRES_AT_MS) return null;
  return timestampMs;
}

function parseQuotaBucket(raw: unknown, fallback?: Partial<QoderCnQuotaBucket>): QoderCnQuotaBucket {
...
export function getQoderCnAccountDisplayEmail(account: QoderCnAccount): string {
  // For Qoder CN, prefer display_name (user's actual name) over email (which may be a UUID)
  return account.display_name || account.email || account.user_id || account.id;
}

export function getQoderCnPlanBadge(account: QoderCnAccount): string {
  return (
    firstNonEmptyString(
...
export function getQoderCnSubscriptionInfo(account: QoderCnAccount): QoderCnSubscriptionInfo {
  const planTag = getQoderCnPlanBadge(account);
...
export function getQoderCnUsage(account: QoderCnAccount): QoderCnUsage {
...
export function getQoderCnUsageOverview(account: QoderCnAccount): QoderCnUsageOverview {
  const subscription = getQoderCnSubscriptionInfo(account);
...
export function hasQoderCnQuotaData(account: QoderCnAccount): boolean {
