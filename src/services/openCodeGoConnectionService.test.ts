...
test('OpenCode Go summaries contain key and email hints but never raw credential material', () => {
  ...
  assert.equal(JSON.stringify(summary).includes('primary@example.com'), false);
});
...
test('OpenCode Go quota normalization preserves partial windows independently', () => {
  const quota = normalizeOpenCodeGoQuotaSnapshot({
    rolling: { status: 'available', usedPercent: 25, remainingPercent: 75, resetsAt: 1_900_000_000 },
    weekly: { status: 'available', usedPercent: 60, remainingPercent: 40, resetsAt: 1_900_100_000 },
    monthly: {
      status: 'error', usedPercent: null, remainingPercent: null, resetsAt: null,
      error: 'window unavailable', responseSecret: 'must-not-escape',
    },
    status: 'partial', queriedAt: 1_800_000_000,
  });
...
test('OpenCode Go quota normalization tolerates missing window fields', () => {
  ...
  assert.deepEqual(quota.rolling, {
    status: 'error', usedPercent: 10, remainingPercent: null, resetsAt: null,
    error: 'reset missing',
  });
  assert.equal(quota.weekly.resetsAt, 1_900_100_000);
