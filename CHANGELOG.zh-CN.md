...
- **WorkBuddy AI 添加账号隔离授权**：每次使用独立 Chrome/Edge 临时数据目录和无痕窗口，避免复用日常浏览器的登录 Cookie；国内版授权流程不变。
- **授权窗口生命周期管理**：授权完成、取消、过期或被新会话替换时关闭本次窗口并尝试清理临时数据；不关闭日常浏览器，找不到支持的浏览器时明确报错。
- **防止失效授权写入账号**：保存凭据前校验当前会话，避免取消或替换后的旧请求导入账号。

### 更新说明

- 沿用 v1.3.52 的独立签名密钥和更新源，已安装本分支 v1.3.52 的用户可通过应用内检查更新升级；尚未切换独立更新源的旧版用户需先手动安装本分支版本。

## [1.3.52] - 2026-09-19

### 新增

- **独立签名自动更新**：xiaolong2438 分支使用专属公钥和本仓库发布源。首次请手动安装本版本，以后通过独立更新通道获取后续版本，不再从上游获取安装包。
- **WorkBuddy AI 独立启动路径**：与国内版 WorkBuddy 分开配置，贯通设置、可执行文件探测和 OAuth 客户端识别。

### 修复

- 手动更新下载入口切换到本仓库；分支仓库的 Homebrew PR 自动化改为按需启用，不再作为独立自动更新的必要步骤。
- 首次发布不再依赖历史更新清单，并修正发布工作流的 YAML 缩进。
- 修复未知应用类型错误提示乱码。
