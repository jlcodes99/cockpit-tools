...
- **修复 Windows 刷新 Codex 额度时的栈溢出**：缩小经过桌面命令桥接的异步任务，并为 Windows 构建预留 8 MiB 主线程栈；Codex 额度刷新统一使用有界调度器，最多同时刷新 2 个账号、等待 16 个账号，同一账号重复请求只执行一次，单次任务超时为 2 分钟，完成后立即清理任务和等待者；保留历史额度及原有刷新后处理行为。
