...
- **平台布局「启用/禁用」开关**：布局编辑器中每个平台条目新增启用开关，并提供「全部启用」/「全部禁用」批量操作。禁用某平台会停止其全部自动活动（自动刷新、token 保活、托盘账号读取），但不影响该条目是否显示。
...
- **布局写入参数向后兼容**：`save_tray_platform_layout` 的隐藏与禁用平台集合改为可选参数，未传时保留已落盘的值，旧版前端仍可正常工作。
